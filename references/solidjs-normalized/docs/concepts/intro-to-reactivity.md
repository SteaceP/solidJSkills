...
Reactive primitives, such as [memos](derived-values/memos.md) can be used to create derived values from signals or other memos, and [effects](effects.md) to create subscribers that use the reactive graph output once it's settled.
...
The UI of a Solid application is built using [JSX](understanding-jsx.md).
...
[`batch`](../reference/reactive-utilities/batch.md) can be used to delay an update so the subscriber runs after each signal has been updated.
