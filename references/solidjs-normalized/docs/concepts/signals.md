...
Signals are the primary means of [managing state](intro-to-reactivity.md#state-management) in your Solid application.
They provide a way to store and update values, and are the foundation of [reactivity](intro-to-reactivity.md) in Solid.
...
You can create a signal by calling the [`createSignal`](../reference/basic-reactivity/create-signal.md) function, which is imported from `solid-js`.
...
When a signal is called within a [tracking scope](intro-to-reactivity.md#tracking-changes), the signal adds the dependency to a list of subscribers.
