...
When dealing with dynamic data, Solid offers two ways to render lists: the [`<For>`](../../reference/components/for.md) and `<Index>` components.
...
Index is a [_signal_](../signals.md) and must be called as a function to retrieve its value.
...
`<For>` is designed to be used when the _order_ and _length_ of the list may change frequently.
