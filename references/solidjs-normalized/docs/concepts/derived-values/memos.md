...
They are similar to [derived signals](derived-signals.md) in that they are reactive values that automatically re-evaluate when their dependencies change.
However, unlike derived signals, memos are optimized to execute only once for each change in their dependencies.

Memos expose a _read-only_ reactive value (like a [signal](../signals.md)) and track changes in their dependencies (similar to an [effect](../effects.md)).
