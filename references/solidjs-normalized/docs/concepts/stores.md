...
Unlike [signals](signals.md), which track a single value and trigger a full re-render when updated, stores maintain fine-grained reactivity by updating only the properties that change.
...
While signals are created using the [`createSignal`](../reference/basic-reactivity/create-signal.md) function and require calling the signal function to access their values, store values can be directly accessed without a function call.
...
[`batch`](../reference/reactive-utilities/batch.md), so all the elements update
