...
Instead, it is recommended to use [createMemo](../reference/basic-reactivity/create-memo.md) to compute new values that rely on other reactive values.
:::
...
This is useful for setting up the effect and initializing variables or subscribing to [signals](signals.md).
...
In situations where you just want to run a side effect **once**, you can use the [`onMount`](../reference/lifecycle/on-mount.md) function.
...
While `onMount` is useful for running a side effect once, [`onCleanup`](../reference/lifecycle/on-cleanup.md) is helpful for cleaning up a task when it is no longer needed.
