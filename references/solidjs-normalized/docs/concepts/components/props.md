...
[`mergeProps`](../../reference/reactive-utilities/merge-props.md) is a Solid utility function designed to merge multiple potentially reactive objects together.
...
[`splitProps`](../../reference/reactive-utilities/split-props.md) is a utility function designed to help split a single props object into multiple sets of props, retaining the reactivity of the individual properties.
...
For instances like these, Solid provides a [`children`](../../reference/component-apis/children.md) helper that ensures you always get the right child components without anything unwanted happening.
...
When multiple components across different levels need access to the same data, Solid’s [Context](../context.md) API provides a cleaner alternative. [Context](../context.md) allows you to supply values to deeply nested components without manually threading props through each layer.
