...
- [`on:__`](../../reference/jsx-attributes/on.md): adds an event listener to the `element`. This is also known as a _native event_.
- [`on__`](../../reference/jsx-attributes/on-star.md): adds an event listener to the `document` and dispatches it to the `element`. This can be referred to as a _delegated event_.
...
Instead of attaching event listeners to every individual element, Solid uses _synthetic event delegation_, through the [`on__`](../../reference/jsx-attributes/on-star.md) form .
...
- [Portals](../control-flow/portal.md) propagate events following the _component tree_ and not the _DOM tree_, making them easier to use.
