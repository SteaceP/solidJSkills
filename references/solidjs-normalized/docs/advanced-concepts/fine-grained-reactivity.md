...
- [Memos](../concepts/derived-values/memos.md) resemble [effects](../concepts/effects.md) but are distinct in that they _return_ a signal and optimize computations through caching.
...
[Signals](../concepts/signals.md) are like mutable variables that can point to a value now and another in the future.
...
In Solid, the [`createSignal`](../reference/basic-reactivity/create-signal.md) function can be used to create a signal.
...
[Effects](../concepts/effects.md) are functions that are triggered when the signals they depend on point to a different value.
