...
The returned object contains a `Provider` component and an optional `defaultValue`; [`useContext`](use-context.md) reads the nearest matching provider value.
...
Default value returned by [`useContext`](use-context.md) when no matching provider is found.
...
- `Context.Provider` passes its `value` prop to descendant calls to [`useContext`](use-context.md).
