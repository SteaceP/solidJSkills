...
This object, created using the [`createMiddleware`](../reference/server/create-middleware.md) function, defines when middleware functions execute throughout the request lifecycle.
...
Other server-side contexts must use the [`getRequestEvent`](../../reference/server-utilities/get-request-event.md) function to access the `event.locals` object.
...
[Solid Router](../../solid-router/index.md) provides the [`redirect` helper function](../../solid-router/reference/response-helpers/redirect.md) which simplifies creating redirect responses.
