...
- `src/routes/` - any files or pages will be located in this directory.
  You can learn more about the [`routes` folder in the routing section](building-your-application/routing.md).
- [`src/entry-client.tsx`](reference/entrypoints/entry-client.md) - this file is what loads and _hydrates_ the JavaScript for our application on the client side (in browser).
  In most cases, you will **not** need to modify this file.
- [`src/entry-server.tsx`](reference/entrypoints/entry-server.md) - this file will handle requests on the server.
