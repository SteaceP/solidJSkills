...
However, this protection does not apply when using [`innerHTML`](../../reference/jsx-attributes/innerhtml.md).
...
If you enforce a strict CSP, configure SolidStart to use JSON serialization mode to avoid `unsafe-eval` requirements. See [defineConfig serialization](../reference/config/define-config.md#serialization). Note that `unsafe-eval` is only required for `serialization.mode: "js"`.
...
1. Create a middleware that configures the CSP header.
   It must then be registered using the [`onRequest`](../advanced/middleware.md#onrequest) event.
