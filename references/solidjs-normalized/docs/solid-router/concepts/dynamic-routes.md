...
When using dynamic segments, the values can be accessed via the [`useParams`](../reference/primitives/use-params.md) primitive within the component.

:::note[Note on Animation/Transitions]
Routes that share the same path match will be treated as the same route.
If you want to force re-render you can wrap your component in a keyed [`Show`](../../concepts/control-flow/conditional-rendering.md) like:
