...
Unlike the [default router](../reference/components/router.md), the hash portion of the URL will not be handled by the server, meaning this is a client-side only routing.
To use hash mode, replace the `<Router />` component in the application with [`<HashRouter />`](../reference/components/hash-router.md).
