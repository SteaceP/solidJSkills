...
Route modules can export a [`preload`](../reference/preload-functions/preload.md) function that receives params, search values, and router context.
...
The [`usePreloadRoute`](../reference/primitives/use-preload-route.md) primitive exposes the same scheduling behaviour for imperative flows like flyout previews, timers, or observer driven experiences.
...
The component API [`lazy()`](../../reference/component-apis/lazy.md) exposes a `preload()` method that resolves a component without rendering it.
