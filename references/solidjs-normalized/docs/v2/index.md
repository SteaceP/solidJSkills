...
| **Reference** | [<Reveal>](reference/components/reveal.md) | Coordinated sequential reveals across multiple `<Loading>` boundaries (replaces `<SuspenseList>`). |
| **Reference** | [flush()](reference/reactive-utilities/flush.md) | Synchronously flush microtask auto-batched reactive updates (replaces `batch()`). |
| **Reference** | [action & createOptimisticStore](reference/mutations/action-and-optimistic.md) | Generator-based mutations and optimistic updates with automatic rollback. |
