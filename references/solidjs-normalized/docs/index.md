...
- **SolidJS 1.x (Production Stable)**: The default documentation found across [`/concepts`](/concepts), [`/reference`](/reference), [`/advanced-concepts`](/advanced-concepts), [`/solid-router`](solid-router/index.md), and [`/solid-start`](solid-start/index.md). Target this version for all active production deployments using `createSignal`, `<Index>`, `<Show>`, `<For>`, and `@solidjs/start`.
- **SolidJS 2.0-rc.9 (Release Candidate)**: The upcoming major release documented in the dedicated [`/v2`](v2/index.md) hub. Covers native async graph reactivity ("Fetch High, Block Low", "Write Sync, Run Async"), `<For keyed={false}>`, `<Loading>` / `<Errored>`, `dynamic()`, and the new Vite start mode.
